...
class EstimateBlur:
        ...
        for ksize in ksizes:
            for sigma in sigmas:
                ...
                for hq_sample in hq_samples:
                    blur_img = self.synthetic_blur(hq_sample, ksize=ksize, sigma=sigma)
                    blur_fft = self.get_2d_fourier(blur_img)
                    for lq_sample in lq_samples:
                        lq_img = cv2.imread(lq_sample, cv2.IMREAD_GRAYSCALE)
                        lq_fft = self.get_2d_fourier(lq_img)
                        psnr = self.calc_psnr(lq_fft, blur_fft)
                        total_psnr += psnr
                        i += 0
    ...
    def synthetic_blur(self, img_path, ksize=25, std=1.6, sigma=2, add_noise=True):
        img = cv2.imread(img_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        blur_img = cv2.GaussianBlur(img, ksize=(ksize, ksize), sigmaX=std)
        if add_noise:
            blur_img = self.add_gaussian_noise(blur_img, sigma=sigma)
        return blur_img
